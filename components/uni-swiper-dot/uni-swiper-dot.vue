<template>
	<view class="uni-swiper__warp">
		...
		<view v-if="mode === 'round'" :style="{'bottom':bottom +'upx'}" class="uni-swiper__dots-box" key='round'>
			<view v-for="(item,index) in info" :class="[index === current&&'uni-swiper__dots-long']" :style="{
		    'width':(index === current? dots.width*2:dots.width ) + 'px','height':dots.height +'px' ,'background-color':index !== current?dots.backgroundColor:dots.selectedBackgroundColor,'border':index !==current ? dots.border:dots.selectedBorder}"
			 :key="index" class="uni-swiper__dots-item " />
		</view>
		...
		data() {
			return {
				dots: {
					...
					selectedBackgroundColor: '#FF9E20',
					selectedBorder: '1px #FF9E20 solid'
				}
			}
		},
		watch: {
